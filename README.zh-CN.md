...
技能是指向本地克隆的符号链接，因此只要 `git pull`，所有工具里已安装的技能会同时更新。只有工作流是复制而非链接，用 `./install.sh --update`（`-Update`）可以一步完成拉取与刷新。
...
这套技能里有三条指令，只要还是文字就在结构上无法被遵守——因为它们要求同一个模型同时扮演两个人。在 Claude Code 里，它们改为以脚本运行：循环和中间结果都在代码里，所以计划和实际执行不会漂移。

| 工作流 | 修的是什么 |
|---|---|
| `/superforge-roast-council` | 五个互不可见的独立上下文批评者 → 每个视角配一个只负责"杀掉站不住的指摘"的怀疑者 → 一个裁判。文字版让一个模型分饰五角，到第四个时它已经读过前三个并倾向于同意 |
| `/superforge-verify-evidence` | 一个 agent 跑验收命令；**另一个从未看过实现的 agent** 给证据打分，并被要求去找"这份证据为什么证明不了该结论" |
| `/superforge-dev-waves` | 在任何东西开始**之前**检查并行任务不会写同一个文件；在还没花掉任何成本时打印每个任务的模型与理由；随后由第二个 agent 验证每一项 |
| `/superforge-freshness` | 重新抓取 `SOURCES.md` 里的每个来源，只报告发生漂移的那些。它只报告，绝不改写 |
| `/superforge-selfcheck` | 读 `docs/superforge-log.md`（也就是你被迫说了两遍的话），把它变成带文件名的修改建议。这是唯一一个靠真实使用、而不是靠想象来改进这套技能的机制 |

四者共有、而提示词做不到的一点：**产出东西的 agent 永远不会给自己打分。**

跑之前值得知道的一件事：工作流里的每个 agent 都会沿用 `/model` 的设置，除非脚本逐阶段指定模型。没有分层的工作流不只是省不下钱，而是**按 agent 数量把浪费放大**。这四个都为每个阶段指定了模型与 effort，并打印明细。

仅限 Claude Code（v2.1.154 及以上）。其他环境下同样的流程以文字方式运行，不会卡住。

---

## 让内容不过期

凡是提到模型名、API 形态或别家厂商指引的内容，都会悄悄过期。一个自信地叫出已经不存在的东西的技能，比什么都不说更糟。

- **`SOURCES.md`** 列出每一条依赖外部的主张，附上核对过的 URL 和日期。方法本身不需要日期："只有写入文件不相交时两个任务才能并行"不会过期，会过期的是模型名和目录路径。
- **`/superforge-freshness`** 会全部重新核对，只报告漂移，并给出可直接粘贴的替换文本。
- **已安装的副本**会自动跟上。`install.sh` 把每个技能符号链接到你的克隆，所以 `git pull` 就能同时刷新所有工具里的全部技能。只有工作流是复制，用 `./install.sh --update`。
- **脱离仓库的副本**（上传到 claude.ai 的 `.skill`、粘贴进某个项目的文件）根本没有更新通道。这正是每条主张都写上日期的原因：文件不知道今天是几号，读它的人一定知道。
