...
| [`superforge-scroll/scripts/plan_map.py`](./skills/superforge-scroll/scripts/plan_map.py) | renders the plan's tables to an SVG floor plan with the camera path drawn on it, and machine-checks that each leg ends where the next begins and that no seam over-turns — stdlib only, chrome labels in en / ja / ko / zh-CN / es |
