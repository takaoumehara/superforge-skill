...
### 🖨️ プランは自己検証する図面になる
`scripts/plan_map.py` がプランの表から SVG を起こします——部屋の輪郭、開口部、番号付きのカメラ経路、レッグごとの向きのティック、北、太陽、スケールバー。ゲート1が「読んでもらえない座標表」ではなく、**クライアントが10秒で可否を判断できる図**になります。描画のついでに、各レッグの終点が次の始点と一致しているか、シームで15°以上曲がっていないかを検証します。すでに書かれている数字の引き算なので、人がやる理由がありません。ラベルは既定で英語、`--lang` で ja / ko / zh-CN / es。部屋名やシーン名はプランに書かれた言語のまま出ます。
