...
### 🗂️ 記憶はリポジトリの中のファイル。手元にあるとは限らないツールではなく
`docs/failforward.md` に、コミットして、追記だけして、仮説を立てる前に読む。効くのは直し方の記録ではなく、**`Looked like`——最初に疑って外したもの**のほう。そこが繰り返すからだ。「重い」という報告が4回続けてインデックス漏れだった、という事実は次にどこを見るかを教えてくれるが、個人の記憶では持ちきれない。

### 🔍 4フェーズが始められないバグのために
「再現して切り分ける」は、再現できることを前提にしている。そして高くつくのは、再現できないほうだ。「たまに」の中身を絞る——タイムゾーンとロケールは、1台の端末から見るとランダムそのものに見える。環境を1変数ずつ合わせる。当てずっぽうより、計測を仕込んで待つ。「前は動いた」なら、コードを考えるのをやめて二分探索する。
