...
### ⚡ 性能和第二语言，是布局阶段的决定，不是以后的问题
首屏视频、四种字重、跑在会触发布局的属性上的动画——全都是在设计阶段定下的，等有人去做性能分析的时候，上面已经压了一层组件。所以三个预算数字要和 token 一起写进 `docs/design.md`，并写清超了怎么办。语言也一样：德语要长出 30–40%，而且**越短的字符串涨得越狠，所以最先坏的是按钮**。别把容器按当前文字来定尺寸——现在做几乎不要钱，以后补就是重做。
