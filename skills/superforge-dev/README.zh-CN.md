...
### 🗄️ 表结构是唯一一个「越成功越难改」的部分
没有用户的代码一个下午就能重写，装着真实数据的表不行。所以那些现在便宜、以后昂贵的决定要有意识地先做掉——不可猜的 ID、UTC 时间戳、以整数最小单位存的金额，以及**每次鉴权都要走一遍的归属链**。还有：数据层的性能问题归根到底就那三种；迁移要用增量方式、跑在生产数据的副本上、并且带着一个你已经试过的回滚。

### 🧱 拆得让并行可以被证明是安全的
拓扑和模型分级都救不了一个糟糕的拆分，而无人值守跑挂的地方，往往正是拆分。每个任务只有一个结果、一条验证命令，以及**它会写入的文件清单**——因为规则是：*两个任务能并行，当且仅当这两组文件不相交。* 不是「应该没事」，而是列出来、并且不相交。共用的地基单独先跑。
