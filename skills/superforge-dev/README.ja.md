...
### 🗄️ スキーマは、成功するほど変えにくくなる唯一の部分
ユーザーのいないコードは午後いっぱいで書き直せる。実データの入ったテーブルはそうはいかない。だから、今なら安く後なら高い決定を意識的に済ませる——推測できないID、UTCのタイムスタンプ、整数の最小単位で持つ金額、そして**権限チェックが毎回たどる所有関係**。加えて、データ由来の遅さの原因は結局3つしかないこと、そして移行は加算的に、本番のコピーに対して、先に試したロールバックを持って走らせること。
