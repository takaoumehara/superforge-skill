...
### 🗄️ 스키마는 성공할수록 바꾸기 어려워지는 유일한 부분
사용자 없는 코드는 오후 한나절이면 다시 쓸 수 있지만, 실제 행이 들어 있는 테이블은 그렇지 않다. 그래서 지금은 싸고 나중은 비싼 결정들을 의식적으로 먼저 해 둔다 — 추측 불가능한 ID, UTC 타임스탬프, 정수 최소 단위로 담는 금액, 그리고 **모든 권한 검사가 타고 가는 소유 관계**. 여기에 데이터 쪽 성능 문제의 원인은 결국 세 가지뿐이라는 것과, 마이그레이션은 덧붙이는 방식으로, 프로덕션 사본에 대해, 이미 시험해 본 롤백을 들고 돌린다는 것까지.
