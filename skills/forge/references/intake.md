...
| アイデアはあるが売れるか不明 | `forge-brain`（検証モード）→ `forge-biz` |
| 何を作るかは決まっている | `forge-ui` → `forge-dev` |
| 動くものがあるが質が低い | `forge-roast` → `forge-ui` |
| 動くものがあり、出したい | `forge-verify` → `forge-biz`（GTM） |
...
> Written by: forge (intake) · Last updated: <YYYY-MM-DD>
> Status: agreed
