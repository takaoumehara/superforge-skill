...
### 🏅 証拠に等級をつけ、「確認しました」は証拠として数えない
再現可能・観測・導出・断言の4等級。導出は必ず、どのコマンドかどの記録に依っているかを書く。そして**検証報告に断言を1件でも混ぜてはいけない。** ここで捕まるのは、結論を測定値の口調で書いてしまう静かな失敗だ——「モバイル表示、確認済み」は証拠ではない。「375px のスクリーンショット、添付」が証拠。`## 確認していないこと` は必須セクション。
