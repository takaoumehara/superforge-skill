...
### 📡 알아챌 수 있는가, 고칠 수 있는가, 되찾을 수 있는가
혼자 만드는 제품의 기본 상태는 **사용자가 모니터링 시스템 역할을 하는 것**이다. 이를 메우는 데 필요한 건 세 가지, 각각 반나절이면 된다 — DB를 건드리는 URL에 대한 가동 확인(정적 헬스 페이지는 안 된다. 정말 중요한 장애에서 계속 초록색이다), 오류 수집, 그리고 「제품이 동작하지 않는다」를 뜻하는 알림 하나. 그리고 멈춘다. 실제로 대응하지 않을 알림을 늘리면 진짜 알림을 무시하는 훈련이 된다. 실제로 해 본 롤백과 실제로 복원해 본 백업은 여기서 마감이 아니라 `BLOCK` 조건이다. 시험하지 않은 백업은 백업이 아니라 파일에 대한 믿음이다.
