...
### 📡 你会不会发现、能不能修、能不能找回
一个人做的产品，默认状态是**用户在替你当监控系统**。补上它只要三样，每样半天：对一个会碰数据库的 URL 做存活检查（不是静态健康页——真正要命的故障里它一直是绿的）、错误上报，以及一条意思是「产品挂了」的告警。然后就停。超出你会去处理的告警量，只会训练你忽略真的那条。演练过的回滚和真正恢复过的备份，在这里是 `BLOCK` 条件而不是加分项：没试过的备份不是备份，是你对某个文件的信念。
