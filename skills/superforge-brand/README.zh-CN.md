...
### 💸 生成的素材到底花了多少钱，以及你能不能用
单次很便宜，恰恰因此总额没人算——真正的成本是为了得到一张而生成的那三十张，而视频按每秒算要高上一到两个数量级。所以在动手之前，就给每个素材定一个迭代次数的预算。真正难的是一致性：一张好图很容易，十二张看起来像同一个品牌才是问题，解法是把参数固定住、在一次会话里做完，而不是把提示词写得更漂亮。最后是三个权利问题——在发布之前答完，而不是之后。以及：绝不生成产品本身，绝不生成客户。
