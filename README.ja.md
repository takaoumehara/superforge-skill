...
スキルはクローンへのシンボリックリンクなので、`git pull` するだけで全ツールのインストール済みスキルが一斉に更新されます。ワークフローだけはリンクではなく複製なので、`./install.sh --update` (`-Update`) を使うと pull と複製の更新を一度に済ませられます。
...
## 5つのワークフロー — 文章では強制できなかった部分のために

このスキル群の指示のうち3つは、文章である限り構造的に守らせようがありませんでした。1つのモデルに2人分の役をやらせているからです。Claude Code ではこれをスクリプトとして走らせます。ループも中間結果もコードの中にあるので、計画と実行がズレようがありません。

| ワークフロー | 何を直すか |
|---|---|
| `/superforge-roast-council` | 互いを一度も見ない5つの独立コンテキストの批評者 → レンズごとに「持ちこたえない指摘を殺す」だけが仕事の懐疑役 → 判定役1人。文章版は1つのモデルに5人を演じさせるので、4人目の時点で前の3人を読んでいて同意してしまう |
| `/superforge-verify-evidence` | proof line を走らせるエージェントと、**実装を一度も見ていない別のエージェント**が採点する。しかも「この証拠では主張を証明できない理由」を探させる |
| `/superforge-dev-waves` | 並列する2タスクが同じファイルを書かないことを、何かが始まる**前**に検査する。まだ何も消費していない時点で全タスクのモデルと理由を印刷し、そのあと各タスクを別のエージェントが証明する |
| `/superforge-freshness` | `SOURCES.md` の全ソースを取り直し、ズレたものだけを報告する。報告はするが、書き換えは絶対にしない |
| `/superforge-selfcheck` | `docs/superforge-log.md`（＝あなたが二度言わされたこと）を読み、ファイル名つきの修正案に変える。想像ではなく実使用からこのスキル群を良くする、唯一の仕組み |

4本に共通する、プロンプトでは保証できない唯一のこと。**何かを作ったエージェントが、それを採点することは絶対にない。**

走らせる前に知っておく価値があること。ワークフロー内の各エージェントは、スクリプトがステージごとにモデルを割り当てない限り `/model` の設定をそのまま使います。つまり階層分けのないワークフローは、節約に失敗するどころか**エージェント数だけ無駄を増幅**します。この4本は全ステージにモデルと effort を割り当て、内訳を印刷します。

Claude Code 専用（v2.1.154 以降）。それ以外の環境では同じループが文章として動き、止まることはありません。

---

## 情報を古びさせないために

モデル名・API の形・他社のガイドラインに触れているものは、黙って古くなります。存在しないものを自信たっぷりに名指しするスキルは、何も言わないスキルより有害です。

- **`SOURCES.md`** — 外部に依存する主張を、検証したURLと日付つきで全部並べたもの。手法には日付が要りません。「書き込むファイルが交差しないときだけ2タスクは並列にできる」は古びません。古びるのはモデル名とディレクトリのパスです。
- **`/superforge-freshness`** — それを全部取り直し、ズレたぶんだけを、貼り付けられる差し替え文つきで報告します。
- **インストール済みのコピー**は自動で追随します。`install.sh` は各スキルをクローンへのシンボリックリンクとして張るので、`git pull` するだけで全ツールのスキルが一斉に更新されます。ワークフローだけは複製なので `./install.sh --update` を使ってください。
- **切り離されたコピー**（claude.ai にアップロードした `.skill`、どこかに貼り付けたファイル）には更新経路がありません。だからこそ各主張に日付を書いてあります。ファイルは今日が何日か知りませんが、読む側は必ず知っているからです。
