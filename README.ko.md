...
스킬은 클론을 가리키는 심볼릭 링크이므로 `git pull`만 해도 모든 도구의 설치된 스킬이 한꺼번에 갱신됩니다. 워크플로만 링크가 아니라 복사본이라, `./install.sh --update`(`-Update`)를 쓰면 pull과 갱신을 한 번에 끝낼 수 있습니다.
...
스킬의 내용은 영어로 쓰여 있습니다. 당신은 그럴 필요가 없습니다.

프로젝트에서 처음 돌 때 딱 한 번, **방금 당신이 쓴 것에서 추측한 답을 채워 넣은 채로** 묻고, 그 뒤로는 묻지 않습니다:

```
대화: 한국어   ← 당신이 쓴 방식에서 추측
docs/ 안의 파일: 한국어

[1] 둘 다 영어로   [2] 대화는 한국어, 파일은 영어   [3] 다른 언어
```

**이 둘을 나눈 것은 의도적입니다.** 한국어로 일하면서 저장소는 해외 사람들과 공유하는 경우, 답변은 한국어에 파일은 영어를 원하기 마련인데, 그걸 먼저 말하는 사람은 거의 없습니다.

답은 `docs/superforge.md`에 저장되고, `/clear` 해도 남고, 인계 캡슐에도 실리며, 「언어 바꿔 줘」 한마디로 언제든 바뀝니다. 질문을 무시하고 바로 용건을 말하면 추측을 채택하고 일을 시작합니다.
...
싸지는지는 **토큰이 어디서 처리되는가**로 갈립니다. 에이전트 개수가 아닙니다.

| 무엇을 부탁했나 | 무슨 일이 일어나나 | 싸지나? |
|---|---|---|
| 「오타 고쳐 줘」 | 당신의 세션이 그냥 고침 | **아니오. 그리고 이게 이미 제일 쌉니다** — 에이전트를 띄우면 오히려 비쌈 |
| 「이 로그 2000줄 요약해 줘」 | 싼 등급의 에이전트 **하나** | **예, 크게** — 덩어리는 싼 모델에서 태우고 결과만 돌아옴 |
| 「이 기능 만들어 줘」(다섯 작업으로 쪼개짐) | 작업마다 다른 등급 | **예. 여기가 본무대** |
| 「아키텍처 정해 줘」 | 제일 좋은 모델로, 위임 없이 | 아니오. 여기는 아낄 자리가 아님 |

그래서 **작업 하나짜리**를 밖으로 낼지는 「작업이 둘 이상인가」가 아니라 **「토큰은 많이 먹는데 판단력은 별로 필요 없는가」**로 정합니다.
...
## 다섯 개의 워크플로 — 글로는 강제할 수 없던 부분을 위해

이 스킬 묶음의 지시 중 셋은, 글인 한 구조적으로 지켜질 수가 없었습니다. 하나의 모델에게 두 사람 몫을 시키기 때문입니다. Claude Code에서는 이제 스크립트로 돕니다. 루프와 중간 결과가 코드 안에 있으니 계획과 실행이 어긋날 수 없습니다.

| 워크플로 | 무엇을 고치나 |
|---|---|
| `/superforge-roast-council` | 서로를 한 번도 보지 않는 다섯 개의 독립 컨텍스트 비평가 → 렌즈마다 "버티지 못하는 지적을 죽이는" 일만 하는 회의론자 → 판정자 하나. 글 버전은 한 모델에게 다섯 명을 연기시키므로, 네 번째에 이르면 앞의 셋을 이미 읽고 동의해 버립니다 |
| `/superforge-verify-evidence` | 한 에이전트가 검증 명령을 실행하고, **구현을 한 번도 보지 않은 다른 에이전트**가 그 출력을 채점합니다. 그것도 "이 증거로는 주장을 증명할 수 없는 이유"를 찾게 시킵니다 |
| `/superforge-dev-waves` | 병렬 태스크가 같은 파일을 쓰지 않는지 무엇이든 시작되기 **전에** 검사하고, 아직 아무것도 쓰지 않은 시점에 모든 태스크의 모델과 이유를 출력한 뒤, 각각을 두 번째 에이전트가 증명합니다 |
| `/superforge-freshness` | `SOURCES.md`의 모든 출처를 다시 가져와 어긋난 것만 보고합니다. 보고만 할 뿐, 결코 고쳐 쓰지 않습니다 |
| `/superforge-selfcheck` | `docs/superforge-log.md`, 즉 당신이 두 번 말해야 했던 것을 읽어 파일명이 붙은 수정안으로 바꿉니다. 상상이 아니라 실제 사용에서 이 묶음을 개선하는 유일한 장치입니다 |

넷 모두가 보장하고 프롬프트는 보장할 수 없는 단 하나: **무언가를 만든 에이전트가 그것을 채점하는 일은 절대 없다.**

돌리기 전에 알아 둘 것. 워크플로 안의 각 에이전트는 스크립트가 단계별로 모델을 지정하지 않는 한 `/model` 설정을 그대로 씁니다. 즉 계층이 없는 워크플로는 절약에 실패하는 정도가 아니라 **에이전트 수만큼 낭비를 증폭**합니다. 이 넷은 모든 단계에 모델과 effort를 지정하고 그 내역을 출력합니다.

Claude Code 전용(v2.1.154 이상). 그 밖의 환경에서는 같은 루프가 글로 돌아가며 막히지 않습니다.

---

## 최신 상태 유지하기

모델 이름, API 모양, 다른 회사의 가이드라인을 언급하는 것은 조용히 낡습니다. 더 이상 존재하지 않는 것을 자신 있게 부르는 스킬은 아무 말도 하지 않는 스킬보다 나쁩니다.

- **`SOURCES.md`** — 외부에 의존하는 모든 주장을, 검증한 URL과 날짜와 함께 나열한 목록. 방법에는 날짜가 필요 없습니다. "쓰는 파일이 겹치지 않을 때만 두 태스크를 병렬로 돌릴 수 있다"는 낡지 않습니다. 낡는 것은 모델 이름과 디렉터리 경로입니다.
- **`/superforge-freshness`** — 전부 다시 확인해 어긋난 것만, 붙여 넣을 수 있는 교체 문구와 함께 보고합니다.
- **설치된 사본**은 알아서 따라옵니다. `install.sh`는 각 스킬을 클론으로 심볼릭 링크하므로 `git pull`만으로 모든 도구의 스킬이 한꺼번에 갱신됩니다. 워크플로만 복사본이라 `./install.sh --update`를 씁니다.
- **분리된 사본**(claude.ai에 올린 `.skill`, 어딘가에 붙여 넣은 파일)에는 갱신 경로가 아예 없습니다. 그래서 모든 주장에 날짜를 적어 둡니다. 파일은 오늘이 며칠인지 모르지만, 읽는 사람은 반드시 알기 때문입니다.
